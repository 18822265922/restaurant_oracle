...
--7.2 查看订单是否全部完成
CREATE OR REPLACE PACKAGE NKCS_ORDER_ITEM_PK 
AS
      TEMP_ID T.ID%TYPE;
      TEMP_STATUS T.MC%TYPE;
END;
CREATE OR REPLACE TRIGGER NKCS_ORDER_ITEM_INS_TRG_ROW_AU 
AFTER UPDATE ON NKCS_ORDER_ITEM 
FOR EACH ROW 
BEGIN
    NKCS_ORDER_ITEM_PK.TEMP_ID:=:OLD.order_id;
    NKCS_ORDER_ITEM_PK.TEMP_STATUS:=:NEW.status;
END;
CREATE OR REPLACE TRIGGER NKCS_ORDER_ITEM_INS_TRG_AU 
AFTER UPDATE ON NKCS_ORDER_ITEM 
DECLARE
    order_item_number          INTEGER;
    order_item_finished_number INTEGER;
BEGIN
    if NKCS_ORDER_ITEM_PK.TEMP_STATUS='finished' then
        select count(*) into order_item_number
        from nkcs_order,nkcs_order_item 
        where nkcs_order.order_id=nkcs_order_item.order_id and nkcs_order.order_id=NKCS_ORDER_ITEM_PK.TEMP_ID;
        select count(*) into order_item_finished_number
        from nkcs_order,nkcs_order_item 
        where nkcs_order.order_id=nkcs_order_item.order_id and nkcs_order.order_id=NKCS_ORDER_ITEM_PK.TEMP_ID and nkcs_order_item.status='finished';
        if order_item_number=order_item_finished_number then
            update nkcs_order set status='completed' where order_id=NKCS_ORDER_ITEM_PK.TEMP_ID;
        end if;
    end if;
END;

--7.3.确定NKCS_ORDER_ITEM状态顺序
